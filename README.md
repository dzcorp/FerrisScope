...
- **Cluster tabs — keep many clusters open at once.** Open several clusters (or virtual contexts) as tabs and switch between them with no loss of state: each tab keeps its own open terminals, AI chats, detail panels, row selection, namespace and table filters. Live sessions survive the switch — a node-exec terminal or streaming chat in a backgrounded tab keeps its connection, it isn't torn down and replayed. Switch from the rail's open-clusters list or the breadcrumb's `Clusters ⌄` dropdown (which also lists every other cluster to open as a new tab). A cluster stays connected only while some tab references it; returning to **Fleet** is a full reset that closes every tab and disconnects all clusters (confirming first if live sessions would be lost). Open tabs are remembered and reopened on next launch (per your Startup behavior); the live session contents are ephemeral and aren't restored.
...
3. **Open more clusters as tabs** from the breadcrumb's `Clusters ⌄` dropdown — switch between them from there or the rail's open-clusters list, with each tab's terminals, chats and panels kept alive. `Clusters` (the crumb itself) returns to Fleet and resets everything.
