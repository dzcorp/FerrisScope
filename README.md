...
- **Cluster tabs — keep many clusters open at once.** Open several clusters (or virtual contexts) as tabs and switch between them with no loss of state: each tab keeps its own terminals, AI chats, open and hidden panels, row selection, namespace and table filters, and table scroll. Only the selected tab fetches; switching back picks up where you left off (watches linger 60 s, so a quick switch is warm). Terminals and chats in background tabs keep their connection. Switch from the rail's open-clusters list or the breadcrumb's `Clusters ⌄` dropdown (which also lists every other cluster to open as a new tab). A cluster stays connected only while some tab references it; returning to **Fleet** is a full reset that closes every tab and disconnects all clusters (confirming first if live sessions would be lost).
- **Session restore.** Open tabs come back on next launch (per your Startup behavior) with their panels, hidden-panel tray, YAML drafts and AI chats (reloaded by session). Shell terminals restart as fresh shells; exec and `kubectl debug` sessions are never re-run on launch.
...
- **Hide, don't lose.** Click outside a panel or press Esc to hide it into the right-edge tray — logs keep streaming and the panel reopens exactly where you left it; only × closes. Each detail panel keeps its own back/forward history as you follow links.
...
- **Notifications panel**, port-forwards panel, namespace picker, bulk action bar, and a chat dock that hides into the panel tray.
...
3. **Open more clusters as tabs** from the breadcrumb's `Clusters ⌄` dropdown — switch between them from there or the rail's open-clusters list, with each tab's terminals, chats and panels kept, and restored on next launch. `Clusters` (the crumb itself) returns to Fleet and resets everything.
4. **Browse** with the rail (left sidebar) — Workloads → Pods, Network → Services, etc. Tables are virtualized so 5 000-pod namespaces stay snappy.
5. **Open a detail panel.** Click any row. Cross-kind links (owner refs, node, service-account, mounted ConfigMaps / Secrets) navigate inline.
6. **Edit live.** ConfigMap, Secret, ResourceQuota, LimitRange, replicas, PVC storage, plus labels / annotations on opted-in kinds — pencil → edit → Save. Conflicts surface a banner with the colliding manager. Or edit the whole manifest in the YAML tab, `kubectl edit`-style.
7. **Talk to the cluster.** Open the AI dock (right side) — the bundled OpenCode Zen free tier works with zero setup, or pick your own provider — and ask "*why is this pod CrashLoopBackOff?*" The agent runs `fs_pod_diagnose`, pulls events, tails logs, and explains.
