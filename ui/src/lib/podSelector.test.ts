...
function row(uid: string, labels?: Record<string, string>): ResourceRow {
  return { uid, name: uid, __labels: labels };
}
...
describe("acceptsPodDelta", () => {
  ...
  it("admits a new pod that matches a plain matchLabels selector", () => {
    expect(acceptsPodDelta(row("p1", { app: "web" }), s, none)).toBe(true);
  });

  it("rejects a new pod that does not match", () => {
    expect(acceptsPodDelta(row("p1", { app: "api" }), s, none)).toBe(false);
  });

  it("drops a known pod once its labels stop matching", () => {
    const known = new Set(["p1"]);
    expect(acceptsPodDelta(row("p1", { app: "api" }), s, known)).toBe(false);
  });
  ...
  describe("with matchExpressions present", () => {
    ...
    it("keeps updating pods the server already vouched for", () => {
      const known = new Set(["p1"]);
      expect(acceptsPodDelta(row("p1", { app: "web" }), expr, known)).toBe(
        true,
      );
    });

    it("refuses to admit an unknown pod", () => {
      expect(acceptsPodDelta(row("p2", { app: "web" }), expr, none)).toBe(
        false,
      );
    });
  });
