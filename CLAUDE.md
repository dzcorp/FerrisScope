...
## Drawers, tray and session restore

- **One drawer per cluster tab.** Detail / logs / compare / inspect are a single `drawer` slot in `ScopeSlice` (`store.ts`), plain data only — never component state. `ResourceTable` renders a detail drawer only when its `kindId` matches the table; App renders the rest.
- **Hide, don't close.** Drawers take an `onMinimize` prop: outside click and Esc park the drawer in `tray` (`minimizeDrawer` / `restoreTrayItem`); only the × closes. There is no minimise button. `PanelTray` shows the tray plus the minimised chat dock on the right edge as an icon column that widens into a named list on hover/focus (the rail's peek pattern, mirrored). A new drawer kind needs a `Drawer` variant, a `drawerKey` case, and a `trayLabel` case. Drawers publish their width with `useDrawerEdge` so the tray docks beside them.
- **Only the selected cluster tab is live.** App mounts just the active tab's view (keyed by tab id); switching unmounts the old one, so its watches, metrics and streams stop (backend watchers linger 60 s, so a quick switch back is warm). Dock terminals/chats are the exception — they stay mounted per tab. A tab's state lives in its slice and is re-applied on mount: kind, namespaces, filter, selection, drawer + tray, `tableScroll` per kind (ResourceTable saves on unmount), and each detail's inner tab/scroll (`DetailPanel` `onLeave` → `saveDrawerView`). Code under a tab reads through `lib/tabScope.tsx` (`useTabSlice`, `useTabKind`, `readTab`) and writes effect-driven state with `writeTab(tabId, …)`, because cleanup runs after the switch has already moved the mirror to the new tab. Parked drawers still use `useTabActive()` to stand down (Esc, drawer width, polling).
- **Esc goes through `useEscLayer`** (`lib/escStack.ts`) — never a raw window `keydown` for Escape. Topmost layer wins; inner handlers that must win call `preventDefault()`.
- **Session restore.** `lib/session.ts` snapshots each tab's dock tabs, drawer and tray into `session.json` (opaque to Rust, 4 MiB cap). Only plain shell terminals restart (never exec / `kubectl debug` — they would act on the cluster unasked); chats reload by session id; YAML buffers over 256 KiB aren't kept, and total YAML is capped at 3 MiB (active tab first) so a save never hits the Rust cap. Restored drawers are validated per variant — a malformed one would crash `drawerKey`. Persistence goes through `createSaver` (`lib/saver.ts`): serialised, debounced, flushed on window close.
